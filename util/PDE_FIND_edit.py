...
def build_linear_system(u, dt, dx, D = 3, P = 3,time_diff = 'poly',space_diff = 'poly',lam_t = None,lam_x = None, width_x = None,width_t = None, deg_x = 5,deg_t = None,sigma = 2):
    ...
    return 0, Theta, rhs_description
...
def build_linear_system_SP(u, kappa, D = 4, P = 0):
    """
    Not general:
    ['', 'u_{x}', 'u_{xx}', 'u_{xxx}', 'u_{xxxx}']
    """

    uhat = np.fft.fft(u)
    u_1x = np.fft.ifft(  (1j) * np.power(kappa,1) * uhat )
    u_2x = np.fft.ifft(  (-1) * np.power(kappa,2) * uhat )
    u_3x = np.fft.ifft( -(1j) * np.power(kappa,3) * uhat )
    u_4x = np.fft.ifft(  (+1) * np.power(kappa,4) * uhat )

    Theta = np.array([np.ones_like(u_1x),u_1x,u_2x,u_3x,u_4x]).T
